...
to crea-presas
  let a random-pxcor ; posiciones fijas para todas las rapidas
  let b random-pycor
  let c random-pxcor ; y para lentas
  let d random-pycor
  create-presas-rapidas npresas-rapidas [
    set color green
    setxy a b
    fd random 4
    set size 1
...
to-report potencial-comida
  ...
  ask presas-disponibles [
    if not any? presas in-radius 1 [
      set comida self  ; está solitaria, la reporta
    ]
  ]
  report comida
end
...
to mueve-depredadores
  ...
  ; depredadores: deambulan lentamente, larga visión
  ; si detectan una presa solitaria cerca, la atacan; si corre, la persigue, si la tiene cerca, se la come
  ; si detectan una presa a menos de una unidad de otras 4, se espera lejos, si se mueven las sigue,
; después de cierto num de ticks "se cansa" de correr o de esperar y se va
@#$#@#$#@
GRAPHICS-WINDOW
210
10
