...
personajes = ['harry', 'hermione', 'ron', 'hagrid', 'dumbledore', 'malfoy', 'dudley']
removed = []
...
def personajes_relacionados(filename):
    ...
    for rel in relaciones:
        ...
        for p in personajes:
            if p in rel:
                G.nodes[p]['tam_nodo'] = G.nodes[p].get('tam_nodo', 0)+1
        if len(rel) >= 2:
            for otro in rel[1:]:
                if rel[0] != otro:
    ...
    for e in G.edges():
        if G[e[0]][e[1]]["relacionado"] < 10:
            borrar.append((e[0], e[1]))
        else:
            print("{} -> {} : {}".format(e[0], e[1], G[e[0]][e[1]]["relacionado"]))
    G.remove_edges_from(borrar)
    return G
...
def grafica(file):
    ...
    nx.draw_networkx_edges(G, pos, alpha=.3, edge_color=width_list*30,
                           width=width_list/20)
    nx.draw_networkx_labels(G, pos, fontsize=14, font_color="k")
    plt.xticks([])
    plt.yticks([])
    plt.title(file)
