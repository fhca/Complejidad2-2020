...
def a_lista(G):
    ...
    return {"nodos": np.array(G.nodes())[orden_nodos],
            "lista": np.array(list(nx.generate_adjlist(G)))[orden_nodos]}
...
def desde(dic, digráfica=False):
    ...
    if isinstance(dic, dict):  # es diccionario?
        ...
        nodos = range(len(dic))
        m = len(dic[0])
        if all(len(dic[i]) == m for i in range(len(nodos))) and set(e for l in dic for e in set(l)) == {0,
                                                                                                        1}:  # reng de la misma long y conteniendo sólo 0,1
            bmat = dic == 1
            n = range(len(nodos))
